...
train_images = np.load(f'C:/Users/ppssa/OneDrive/Työpöytä/OPINNOT/Kandi/cifar-10-edited-{edit_percent}/cifar10_train_images.npy')
train_labels = np.load(f'C:/Users/ppssa/OneDrive/Työpöytä/OPINNOT/Kandi/cifar-10-edited-{edit_percent}/cifar10_train_labels.npy')
...
# --- SAVE MODIFIED DATA ---
output_dir =  f'modified_cifar10_cat_{edit_percent}'
os.makedirs(output_dir, exist_ok=True)

np.save(os.path.join(output_dir, f'cifar10_train_images_cat_{edit_percent}.npy'), modified_images)
np.save(os.path.join(output_dir, f'cifar10_train_labels_cat_{edit_percent}.npy'), modified_labels)

# Save some of the changed images for reference
image_subdir = os.path.join(output_dir, 'cat_examples')
os.makedirs(image_subdir, exist_ok=True)

for i in indices_to_edit[:100]:
    img = Image.fromarray(modified_images[i].astype('uint8'))
    img.save(os.path.join(image_subdir, f'cat_img_{i}.png'))
